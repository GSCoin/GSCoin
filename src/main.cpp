...
uint256 hashGenesisBlock("0x793dbad088ca412dc51019381f21b4bac7d19dfb3680a117f36800009a9ea765");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GSCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GSCoin Signed Message:\n";
...
{
    // GSCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight <= 17282)
	  ...
	  else if(nHeight <= 34563)
        {
                nSubsidy = 40 * COIN;
        }
	    else if(nHeight <= 522724)
        {
                nSubsidy = 20 * COIN;
        }
...
static const int64 nTargetTimespan = 4 * 60 * 60; // GSCoin: 4 hours
static const int64 nTargetSpacing = 60; // GSCoin: 60 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // GSCoin: per 20blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GSCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // GSCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x793dbad088ca412dc51019381f21b4bac7d19dfb3680a117f36800009a9ea765");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "GSCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fbead94227094183bc711a34fbf41aba794f33408680197841fd6ac0200e0ea01bbc7403e72b8c2356f0397d070709adb1382f7816abaabddb9a34ab0a75ff66") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1394370000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1055498;
        ...
        assert(block.hashMerkleRoot == uint256("0x56e18df558483c4d35daef47dd443f4c2fdf8f993ced868a0ec5d580829a807a"));
            block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GSCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GSCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GSCoinMiner : ProcessBlock, block not accepted");
    }
